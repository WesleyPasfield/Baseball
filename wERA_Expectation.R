...
atbat$date <- as.Date(atbat$date, format = '%Y_%m_%d')
atbat <- filter(atbat, date >= '2016-01-01')
run <- dbGetQuery(db, 'select * from runner')
run$checker <- as.numeric(substr(run$gameday_link, 5,8))
run <- filter(run, checker == 2016)
pr <- dbGetQuery(db, 'select * from action')
pr$checker <- as.numeric(substr(pr$gameday_link, 5, 8))
pr <- filter(pr, checker == 2016)
pitchfull <- dbGetQuery(db, 'select * from pitch')
pitchfull$checker <- as.numeric(substr(pitchfull$gameday_link, 5, 8))
pitchfull <- filter(pitchfull, checker == 2016)
...
pitchScore <- atbat1 %>%
  ##filter(run == 1) %>%
  select(pitcher, mlb_id, run, inning, gameday_link, num.x, num.y) %>%
  ...
  select(inning, mlb_id, run, gameday_link, num.x, num.y, pitcher.y)
...
atbat_lookup <- atbat %>%
  select(gameday_link, inning_side, inning, batter, num, o, inning) %>%
  arrange(gameday_link, inning_side, num) %>%
  mutate(ab_num = row_number())
...
## Calculate odds of scoring in each situation where runners are inherited
...
## Calculate odds for pitcher in game whole inning

inning_end <- group_by(atbat, gameday_link, inning, inning_side) %>%
  summarise(max_num = max(num)) %>%
  as.data.frame() %>%
  inner_join(atbat, by = c('gameday_link' = 'gameday_link', 'inning' = 'inning', 'inning_side' = 'inning_side', 'max_num' = 'num')) %>%
  select(gameday_link, inning, inning_side, pitcher, o, num)

inning_start <- group_by(atbat, gameday_link, inning, inning_side) %>%
  summarise(min_num = min(num)) %>%
  as.data.frame() %>%
  inner_join(atbat, by = c('gameday_link' = 'gameday_link', 'inning' = 'inning', 'inning_side' = 'inning_side', 'min_num' = 'num')) %>%
  select(gameday_link, inning, inning_side, pitcher, o, num)
  

  
### Calculate all runs scored in these innings to determine probability of runs scoring for pitcher who completes full inning
## Then calc expected vs. actual runs scored
...
## Calculate average runs scored in an inning when a new pitcher enters
## Use existing calcs to allocate runs for inherited runners

run_trad <- run %>%
  mutate(run = ifelse(score == 'T' & earned == 'T', 1, 0)) %>%
  mutate(run = ifelse(is.na(run), 0, run)) %>%
  group_by(inning_side, inning, gameday_link) %>%
  summarise(runs = sum(run)) %>%
  as.data.frame() %>%
  mutate(runs = ifelse(is.na(runs), 0, runs)) %>%
  select(inning_side, inning, gameday_link, runs)

eligible_innings <- inning_start %>%
  inner_join(inning_end, c('gameday_link' = 'gameday_link', 'inning' = 'inning', 'inning_side' = 'inning_side', 'pitcher' = 'pitcher')) %>%
  inner_join(run_trad, c('gameday_link' = 'gameday_link', 'inning' = 'inning', 'inning_side' = 'inning_side')) %>%
  mutate(variable = 'normal', outs = 0, inherit_pitcher = 0) %>%
  group_by(variable, outs, inherit_pitcher) %>%
  summarise(replaced_pitcher = sum(runs) / n()) %>%
  as.data.frame() %>%
  arrange(variable, outs, replaced_pitcher, inherit_pitcher)
...
ip_ab <- ip_lookup %>%
  inner_join(atbat_lookup, by = c('ab_num' = 'ab_num')) %>%
  select(start, inning, gameday_link, batter, num)

no_out <- run1 %>%
  select(mlb_id, num, run, inning, gameday_link) %>%
  inner_join(ip_ab, c('inning' = 'inning', 'mlb_id' = 'batter', 'gameday_link' = 'gameday_link')) %>%
  filter(num.y <= num.x) %>%
  filter(substr(start,3,4) == '') %>%
  mutate(run = ifelse(is.na(run), 0, 1))

one_out <- run1 %>%
  select(mlb_id, num, run, inning, gameday_link) %>%
  inner_join(ip_ab, c('inning' = 'inning', 'mlb_id' = 'batter', 'gameday_link' = 'gameday_link')) %>%
  filter(num.y <= num.x) %>%
  filter(substr(start,3,4) == '33') %>%
  mutate(run = ifelse(is.na(run), 0, 1))
...
two_out <- run1 %>%
  ...
  inner_join(ip_ab, c('inning' = 'inning', 'mlb_id' = 'batter', 'gameday_link' = 'gameday_link')) %>%
  filter(num.y <= num.x) %>%
  filter(substr(start,3,4) == '66') %>%
  mutate(run = ifelse(is.na(run), 0, 1))

no_out <- c('none', 0, sum(no_out$run) / nrow(no_out), 0)
one_out <- c('none', 1, sum(one_out$run) / nrow(one_out), 0)
two_out <- c('none', 2, sum(two_out$run) / nrow(two_out), 0)

final_odds_noinh <- as.data.frame(rbind(no_out,rbind(one_out, two_out)))
colnames(final_odds_noinh) <- c('variable', 'outs', 'replaced_pitcher', 'inherit_pitcher')
final_odds <- as.data.frame(rbind(final_odds, final_odds_noinh))
...
datatable(head(all_runs_sp_victims, 20))
datatable(head(all_runs_rp_lucky, 20))
...
## Need to assign probability of scoring for batters that a relief pitcher faces 
## in an inning but does NOT inherit (odds in final_odds)
## Runs that inherit should calc same way as before
## When pitcher starts inning apply the flat rate already calc'd